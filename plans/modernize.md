...
- [ ] Phase 1–2 までを v7.2.x として一度リリースするか?(IE 互換のままインフラだけ整備した版を出すと利用者の確認材料になる)
- [ ] `homepage/` 配下のデモページもモダン化対象に含めるか?(本計画は対象外で進める提案)
- [ ] `examples/` `benchmark/` `debug/` のメンテ方針 (本計画では現状維持)
