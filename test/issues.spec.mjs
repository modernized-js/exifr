...
describe('issues (special cases)', () => {
	...
	it(`#2 - 001.tif`, async () => {
		const output = await exifr.parse(await getFile('001.tif'))
	...
	it(`#2 - 002.tiff with value type 13 (IFD pointer) instead of 4`, async () => {
		...
		const output = await exifr.parse(await getFile('002.tiff'), options)
		assert.exists(output.gps)
	})

	it(`#31 IPTC BUG: RangeError_Offset_outside_bounds.jpg`, async () => {
		const input = await getPath('RangeError_Offset_outside_bounds.jpg')
		const options = {
			tiff: false,
			iptc: true
		}
	...
	it(`#31 ICC BUG: RangeError_Invalid_typed_array_length.jpg`, async () => {
		const input = await getPath('RangeError_Invalid_typed_array_length.jpg')
		const options = {
			tiff: false,
			icc: true
		}
	...
	it(`#35 - path input`, async () => {
		const input = await getPath('issue-exifr-35.heic')
	...
	it(`#41 - IPTC flag true`, async () => {
		const options = {iptc: true, xmp: true}
	...
	it(`#41 - IPTC flag false`, async () => {
		const options = {iptc: false, xmp: true}
		const buffer = await getFile(`issue-exifr-41-Error_Segment_Unreachable.jpg`)
		await exifr.parse(buffer, options)
	})

	it(`#35 - file input`, async () => {
		const input = await getFile('issue-exifr-35.heic')
		const output = await exifr.parse(input, true)
		assertOutputWithoutErrors(output)
		assert.equal(output.Make, 'samsung')
	})
	...
	it(`#36 - ExifImageHeight: Uint16Array(2) [78, 0]`, async () => {
		const input = await getFile('issue-exifr-36.jpg')
		const output = await exifr.parse(input, true)
	...
	it(`#44 - ICC RangeError`, async () => {
		const input = await getFile('issue-exifr-44.jpeg')
		const output = await exifr.parse(input, {icc: true})
		assert.equal(output.ProfileCreator, 'Monaco Systems')
	})

	if (isNode) it(`#46 - FsReader closes FH if reading fails`, async () => {
		const exr = new exifr.Exifr
		try {
			const input = getPath('BonTonARTSTORplusIPTC.xmp')
	...
	it(`#59 - Region (incorrect XMP parsing)`, async () => {
		const input = await getFile('issue-exifr-59.jpg')
		const output = await exifr.parse(input, {xmp: true})
	...
	it(`fast-exif #2 - should not skip exif if 0xFF byte precedes marker`, async () => {
		const output = await exifr.parse(await getFile('issue-fast-exif-2.jpg'), true)
		assertOutputWithoutErrors(output)
	...
	it(`exif-js #124`, async () => {
		const output = await exifr.parse(await getFile('issue-exif-js-124.tiff'), true)
		assertOutputWithoutErrors(output)
	...
	it(`metadata-extractor #152 jpg`, async () => {
		const input = await getFile('issue-metadata-extractor-152.jpg')
		const options = {tiff: true, xmp: true, mergeOutput: true}
		const output = await exifr.parse(input, options)
